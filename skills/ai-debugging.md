...
7. Commit the fix and push it, citing the issue in the commit message (e.g. `Fixes #12` so GitHub links and closes it). Never add a `Co-Authored-By` trailer.
8. Only after the fix is verified, committed, and pushed: call `delete_debug_report` with the report's id, and make sure the GitHub issue is closed — `Fixes #N` closes it automatically when the commit lands on the default branch; otherwise close it with `gh issue close <n> --comment "<what was done>"`. If you could not resolve the report, leave it in place, append what you learned to its description with `save_debug_report`, and leave the issue open with a comment on where the investigation stands.
