...
function OverviewSection({
  ...
  const [thumbBusy, setThumbBusy] = useState<'' | 'upload' | 'generate'>('')
  ...
  const generateThumb = async () => {
    setThumbBusy('generate')
    setError('')
    try {
      const t = await GenerateProjectThumbnail(project.id, '', '')
      if (t.file) await persist({thumbnailFile: t.file})
    } catch (err) {
      setError(
        err instanceof Error && err.message
        ...
        <div className="overflow-hidden rounded-xl border border-edge bg-surface">
          ...
          <div className="flex items-center gap-2 p-2">
            ...
            <button
              ...
              disabled={thumbBusy !== ''}
              title="Generate a cover image with AI from the title and description (OpenAI connection required)"
              className="inline-flex flex-1 items-center justify-center gap-1.5 rounded-lg border border-edge px-2 py-1.5 text-xs font-semibold text-fg transition-colors hover:bg-surface-hover disabled:opacity-50"
            >
              {thumbBusy === 'generate' ? (
                <Loader2 size={12} aria-hidden className="animate-spin" />
