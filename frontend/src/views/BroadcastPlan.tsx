...
export function BroadcastPlan({
  ...
  // The thumbnail with its hover edit CTA, shared by the two placements: it
  // floats inside the description below xl and leads the right column on xl.
  // Each placement's wrapper carries the group/relative classes.
  const thumbBlock = (
    <>
      {thumb.url ? (
        <img
  ...
  return (
    <div className="flex flex-col">
      ...
      <div className="flex max-w-3xl flex-col gap-6 xl:max-w-6xl">
        ...
        {/* Description on the left, the image and channels on the right.
            Below xl the image floats in the description so the text wraps
            it, with the channels following at the bottom; on xl the page
            splits left/right — content left, the image with the channels
            stacked beneath it on the right. Hovering the image offers the
            edit CTA (upload or AI-generate). */}
        <div className="flex flex-col gap-6 xl:flex-row xl:items-start xl:gap-8">
          <div className="min-w-0 text-sm text-fg-muted xl:flex-1">
            <div className="group relative float-right mb-2 ml-4 w-72 max-w-[50%] xl:hidden">
              {thumbBlock}
            </div>
            {plan.description && (
          ...
          <div className="flex flex-col gap-3 xl:w-96 xl:shrink-0">
            <div className="group relative hidden xl:block">{thumbBlock}</div>
